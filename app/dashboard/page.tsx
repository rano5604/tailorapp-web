...
const photoOf  = (o: any, base: string): string | null => {
    const raw =
...
export default async function Dashboard({
    ...
    // 1) Summary
    const summaryRes = await fetch(`${API_BASE}/api/dashboard/summary`, { headers, cache: 'no-store' })
    ...
    const recentUrl = `${API_BASE}/api/orders/shop/${shopId}?limit=5&page=0`
    ...
    const recentItems = uniqueBy(
        recentRaw.sort((a, b) => timeOf(b) - timeOf(a)), // newest first if timestamps exist
        orderKey
    ).slice(0, 5)
                    ...
                    const inner = (
                        <>
                            <div className={styles.cardValue} style={{ color: c.color ?? '#111827' }}>
                                {c.value ?? <span style={{ opacity: .3 }}>—</span>}
                            </div>
                            <div className={styles.cardLabel}>{c.label}</div>
                        </>
                    )
                    return c.href ? (
                        <Link key={i} href={c.href} className={`${styles.card} ${styles.cardLink}`}>{inner}</Link>
                    ) : (
                        <div key={i} className={styles.card}>{inner}</div>
                    )
                })}
            </section>
            ...
            <section className={styles.section}>
                ...
                <ul className={styles.recentList}>
                    {recentItems.length === 0 ? (
                        ...
                        recentItems.map((o, i) => {
                            const name  = nameOf(o)
                            const photo = photoOf(o, API_BASE)
                            return (
                                <li key={o.id ?? o.orderId ?? i} className={styles.recentItem}>
                                    ...
                                    <div className={styles.recentBody}>
                                        ...
                                        {dueOf(o) && <div className={styles.recentMeta}>Delivery {String(dueOf(o)).slice(0, 10)}</div>}
                                    </div>
