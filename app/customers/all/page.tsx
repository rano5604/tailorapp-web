...
import styles from '../../orders/Orders.module.css' // reusing your orders list styles
...
const displayGender = (c: Customer) => c.gender ? String(c.gender).toUpperCase() : ''
const photoOf = (c: Customer): string | null => {
    if (!c.photo || typeof c.photo !== 'string') return null
    return c.photo.startsWith('http') ? c.photo : `${API_BASE}${c.photo.startsWith('/') ? '' : '/'}${c.photo}`
}
...
export default async function CustomersAllPage({
    ...
    // API: /api/customers/by-shop/{shopId}?page&limit (limit/page optional if your API supports)
    const params = new URLSearchParams()
    if (!Number.isNaN(page))  params.set('page', String(page))
    if (!Number.isNaN(limit)) params.set('limit', String(limit))
    const qs  = params.toString() ? `?${params.toString()}` : ''
    const url = `${API_BASE}/api/customers/by-shop/${shopId}${qs}`

    const res = await fetch(url, { headers, cache: 'no-store' })
    if (res.status === 401 || res.status === 403) {
        redirect(`/login?redirect=${encodeURIComponent(`/customers/all?shopId=${shopId}`)}`)
    }
    if (!res.ok) throw new Error(`Failed to load customers (${res.status})`)

    const payload = (await res.json().catch(() => null)) as ApiResponse<Customer> | null
    ...
    return (
        <main className={styles.page}>
            ...
            <section className={styles.titleSection}><h2 className={styles.pageTitle}>Customers</h2></section>
