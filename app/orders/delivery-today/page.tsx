// app/orders/delivery-today/page.tsx
import { cookies } from 'next/headers';
import { redirect } from 'next/navigation';
import Link from 'next/link';
import Avatar from '@/components/Avatar'; // adjust import if you don't use '@'
...
export default async function Page({
    ...
    if (!accessToken && !jSessionId && !tbAuth) {
        redirect('/login?redirect=/orders/delivery-today');
    }
    ...
    // API call (Delivery Today)
    const url = api(`/api/dashboard/orders/today?shopId=${shopId}&page=${page}&limit=${limit}`);
    const res = await fetch(url, { headers, cache: 'no-store' });

    if (res.status === 401 || res.status === 403) {
        redirect(`/login?redirect=${encodeURIComponent(`/orders/delivery-today?shopId=${shopId}`)}`);
    }
    if (!res.ok) {
        throw new Error(`Failed to load today's delivery orders (${res.status})`);
    }
    ...
    return (
        <main className={styles.page}>
            ...
            <section className={styles.titleSection}>
                <h2 className={styles.pageTitle}>Delivery Today</h2>
            </section>
