...
export default function DeliveryTodayPage() {
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                ...
                const url = api(`/api/dashboard/orders/today?shopId=${encodeURIComponent(shopId)}&${qs}`);
                ...
                if (!res.ok) {
                    ...
                    throw new Error(t || `Failed to load delivery today orders (${res.status})`);
                }
                ...
                if (e?.name !== 'AbortError') setErr(e?.message || 'Failed to load delivery today orders');
            } finally {
                setLoading(false);
            }
        }
    ...
    useEffect(() => {
        ...
        p.set('limit', String(limit));
        p.set('page', String(page));
        const href = `/orders/delivery-today?${p.toString()}`;
    ...
    return (
        <main style={pageWrap}>
            ...
            {loading ? (
                ...
                <OrdersList title="Delivery Today" items={orders} emptyText="No Order Found" />
            )}
