...
export default function ConfirmOrderPage() {
    ...
    const working = state.itemId
        ? [{
            ...
            makingCharge: typeof state.makingCharge === 'number' ? state.makingCharge : Number(state.makingCharge) || 0,
    ...
    const total = itemsAll.reduce((sum: number, it: any) => sum + (Number(it.makingCharge) || 0), 0)
    ...
    const urlFor = (it: any, key: 'orderCloth' | 'designPhoto' | 'patternPhoto' | 'measurementCloth' | 'designSketch'): string | undefined =>
        it.photoUrls?.[key] ?? (state as any).photoUrls?.[key] ?? undefined
    ...
    function buildApiPayload() {
        return {
            ...
            paidAmount: advance,
            // trialDate: state.trialDate ?? undefined,
            ...
            items: itemsAll.map((it: any) => {
                ...
                return {
                    ...
                    measurementGroups: [
                        {
                            // specialInstruction: state.specialRequest ?? undefined,
                            clothPhoto: clothUrl,
                            patternClothPhoto: patternUrl || undefined,
                            measurementClothPhoto: measureUrl || undefined,
                            designDrawingPhoto: designUrl || undefined,
                            deliveryDate: (it.urgentDelivery ? it.deliveryDate : state.remainingDeliveryDate) ?? undefined,
    ...
    async function onConfirm() {
        ...
        try {
            ...
            // { status: "success", message, data: { orderId: "1-20251106-0001", id: 118, ... } }
            const body = await res.json().catch(() => ({} as any))
            const data = body?.data ?? body ?? {}

            // Save payload for Success page bill
            try { sessionStorage.setItem('create-order:last', JSON.stringify(data)) } catch {}

            const returnedOrderId: string | undefined = data.orderId || data.code || data.number
            ...
            // Do NOT clear cache here — Success page’s Back to Home will clear it.
        } catch (err: any) {
            ...
            <section className={`${styles.formSection} ${styles.container}`}>
                ...
                <div className={styles.panel}>
                    ...
                    <div className={styles.kvRow}>
                        ...
                        <div className={styles.kvValueAccent}>{dateLong(state.remainingDeliveryDate)}</div>
                    </div>
                        ...
                        <div className={styles.kvValueAccent}>
                            {itemsAll.filter((it: any) => it.urgentDelivery).length === 1 ? (
                                ...
                                <div className={styles.urgentList}>
                                    {itemsAll.filter((it: any) => it.urgentDelivery).map((u: any, i: number) => (
                ...
                {apiError && <div className={styles.error} style={{ marginTop: 10 }}>{apiError}</div>}
            </section>

            {/* Footer */}
            <footer className={`${styles.footer} ${styles.container}`}>
                <button type="button" className={styles.primary} onClick={onConfirm} disabled={submitting}>
