...
import Avatar from '@/components/Avatar'; // adjust import if you don't use the @ alias
...
export default async function AllOrdersPage({
    ...
    const url = api(`/api/orders/shop/${shopId}?limit=${limit}&page=${page}`);
    const res = await fetch(url, { headers, cache: 'no-store' });
    if (res.status === 401 || res.status === 403) {
        redirect(
            `/login?redirect=${encodeURIComponent(`/orders/all?shopId=${shopId}`)}`
        );
    }
