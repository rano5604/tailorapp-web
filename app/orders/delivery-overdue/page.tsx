// app/orders/delivery-overdue/page.tsx
import { cookies } from 'next/headers';
import { redirect } from 'next/navigation';
import Link from 'next/link';
import Avatar from '@/components/Avatar'; // adjust if you don't use '@' alias
import styles from '../Orders.module.css';
import { api, API_ORIGIN } from '@/lib/apiBase';

export const runtime = 'nodejs';         // Buffer for JWT decode
...
export default async function Page({
    ...
    if (!accessToken && !jSessionId && !tbAuth) {
        redirect('/login?redirect=/orders/delivery-overdue');
    }
    ...
    // API call (unified base)
    const url = api(
        `/api/dashboard/orders/overdue?shopId=${shopId}&page=${page}&limit=${limit}`
    );
    const res = await fetch(url, { headers, cache: 'no-store' });

    if (res.status === 401 || res.status === 403) {
        redirect(
            `/login?redirect=${encodeURIComponent(`/orders/delivery-overdue?shopId=${shopId}`)}`
        );
    }
    if (!res.ok) {
        throw new Error(`Failed to load overdue orders (${res.status})`);
    }
    ...
    return (
        <main className={styles.page}>
            ...
            <section className={styles.titleSection}>
                <h2 className={styles.pageTitle}>Delivery Overdue</h2>
            </section>
